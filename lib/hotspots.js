...
export async function collectHotspots(forDate) {
  ...
  // === 获取今天的数据 ===
  // 并行获取外部数据 + 5秒整体超时，超时时只返回推荐池
  let baiduData = [], aihotData = [];
  ...
  try {
    await Promise.race([
      (async () => {
        ...
        baiduData = results[0].status === 'fulfilled' ? results[0].value : [];
        aihotData = results[1].status === 'fulfilled' ? results[1].value : [];
      })(),
      new Promise((_, reject) => setTimeout(() => reject(new Error('外部API超时')), 5000)),
    ]);
  } catch (e) {
    console.warn('外部热点API超时，使用推荐池兜底:', e.message);
  }
