...
async function fetchBaiduHotList() {
  try {
    const data = await fetchJSON('https://top.baidu.com/api/board?tab=realtime', 6000, {
...
async function fetchAihotNews() {
  try {
    const data = await fetchJSON('https://aihot.virxact.com/api/public/items?mode=selected&take=30', 6000);
